...
;(function ($, window, MapSVG, Math) {
  ...
  MapSVG.isPhone = window.matchMedia(
    'only screen and (min-device-width: 320px) and (max-device-width: 812px)'
  ).matches
  ...
  MapSVG.Controller = function (options) {
    ...
    this.scrollable =
      options.scrollable === undefined ? true : options.scrollable
    this.withToolbar =
      options.withToolbar === undefined ? true : options.withToolbar
    this.templates = {
  ...
  MapSVG.Controller.prototype.render = function () {
    ...
    this.view = $('<div />')
      ...
      .addClass('mapsvg-controller-view')
    ...
    if (this.width) this.view.css({ width: this.width })
    if (this.color) this.view.css({ 'background-color': this.color })
  ...
  MapSVG.Controller.prototype.redraw = function () {
    ...
    if (this.templates.toolbar) this.toolbarView.html(this.templates.toolbar)

    this.updateTopShift()

    if (this.noPadding) this.contentView.css({ padding: 0 })
  ...
  MapSVG.DirectoryController.prototype.getToolbarTemplate = function () {
    ...
    if (this.search) {
      ...
      t +=
        '<input class="mapsvg-directory-search" placeholder="{{options.menu.searchPlaceholder}}" />'
      t += '</div>'
    }
  ...
  MapSVG.DirectoryController.prototype.viewDidLoad = function () {
    ...
    if (
      this.mapsvg.getData().options.filters &&
      this.mapsvg.getData().options.filters.on
    ) {
      _this.setFilters()
    }

    if (this.mapsvg.getData().options.colors.directorySearch) {
      this.toolbarView.css({
        'background-color':
  ...
  MapSVG.DirectoryController.prototype.setEventHandlers = function () {
    ...
    this.view
      .on('click.menu.mapsvg', '.mapsvg-directory-item', function (e) {
        ...
        if (
          _this.mapsvg.getData().options.actions.directoryItem.click.showDetails
        ) {
        ...
        if (regions && regions.length > 0) {
          if (_this.mapsvg.getData().options.actions.directoryItem.click.zoom) {
            _this.mapsvg.zoomTo(
              regions,
              _this.mapsvg.getData().options.actions.directoryItem.click
                .zoomToLevel
            )
          }
        ...
        _this.events['click'] &&
          _this.events['click'].call($(this), e, eventObject, _this.mapsvg)
        ...
        if (regions && regions.length) {
          ...
          regions.forEach(function (region) {
            if (region && !region.disabled) {
              _this.mapsvg.getData().options.mouseOver &&
                _this.mapsvg.getData().options.mouseOver.call(region, e, _this)
            }
          })
        }
        _this.events['mouseover'] &&
          _this.events['mouseover'].call($(this), e, eventObject, _this.mapsvg)
      })
      .on('mouseout.menu.mapsvg', '.mapsvg-directory-item', function (e) {
        ...
        if (regions && regions.length) {
          ...
          regions.forEach(function (region) {
            if (region && !region.disabled) {
              _this.mapsvg.getData().options.mouseOut &&
                _this.mapsvg.getData().options.mouseOut.call(region, e, _this)
            }
          })
        }

        _this.events['mouseout'] &&
          _this.events['mouseout'].call($(this), e, eventObject, _this.mapsvg)
      })
  ...
  MapSVG.DirectoryController.prototype.getRegion = function (id) {
    ...
    if (_this.mapsvg.getData().options.menu.source == 'regions') {
      ...
      if (obj.region_id) region = _this.mapsvg.getRegion(obj.region_id)
    }
    return region
  }
  MapSVG.DirectoryController.prototype.setFilters = function () {
    ...
    if (
      !_this.formBuilder &&
      _this.mapsvg.getData().options.filters &&
      _this.mapsvg.getData().options.filters.on
    ) {
    ...
    if (
      (_this.mapsvg.getData().options.filters &&
        _this.mapsvg.getData().options.filters.on) ||
      (_this.database.query.filters &&
        Object.keys(_this.database.query.filters).length > 0)
    ) {
      for (var field_name in _this.database.query.filters) {
        ...
        if (_this.mapsvg.getData().options.filters.on && filterField) {
          filters
            .find('[data-parameter-name="' + _field_name + '"]')
            .val(field_value)
        } else {
  ...
  MapSVG.DetailsController.prototype.getToolbarTemplate = function () {
    if (this.withToolbar) return '<div class="mapsvg-details-close"></div>'
    else return ''
  }
  ...
  MapSVG.DetailsController.prototype.viewDidLoad = function () {
    ...
    this.events &&
      this.events['shown'] &&
      this.events['shown'].call(_this, _this.mapsvg)
  }
  ...
  MapSVG.DetailsController.prototype.setEventHandlers = function () {
    ...
    this.view.on('click', '.mapsvg-details-close', function () {
      ...
      _this.events &&
        _this.events['closed'] &&
        _this.events['closed'].call(_this, _this.mapsvg)
    })
  }
  ...
  MapSVG.PopoverController.prototype.viewReadyToFill = function () {
    var _this = this
    this.resizeSensor = new MapSVG.ResizeSensor(
      this.contentSizer[0],
      function () {
        _this.adjustHeight()
        _this.updateScroll()
        _this.events['resize'] &&
          _this.events['resize'].call(_this, _this.mapsvg)
      }
    )
  }
  ...
  MapSVG.PopoverController.prototype.adjustHeight = function () {
    ...
    _this.container.height(
      _this.container.find('.mapsvg-auto-height').outerHeight() +
        _this.toolbarView.outerHeight()
    )
  }
  MapSVG.PopoverController.prototype.adjustPosition = function () {
    ...
    _this.container.css({
      transform:
        'translateX(-50%) translate(' + pos[0] + 'px,' + pos[1] + 'px)',
    })
  }
  ...
  MapSVG.PopoverController.prototype.setEventHandlers = function () {
    ...
    this.view.on('click', '.mapsvg-popover-close', function () {
      _this.close()
    })

    $('body').one(
      'mouseup.popover.mapsvg touchend.popover.mapsvg ',
      function (e) {
        setTimeout(function () {
          if (
            _this.mapsvg.getData().isScrolling ||
            $(e.target).closest('.mapsvg-popover').length ||
            $(e.target).hasClass('mapsvg-btn-zoom')
          )
            return
          _this.close()
        }, 50)
...
;(function ($) {
  ...
  MapObject.prototype.getBBox = function () {
    ...
    if (!(this instanceof Marker)) {
      var matrix = this.node[0].getTransformToElement(
        this.mapsvg.getData().$svg[0]
      )
      ...
      if (typeof data == 'string') {
        if (data.substr(0, 1) == '[' || data.substr(0, 1) == '{') {
          try {
            ...
            var error =
              'MapSVG: Error in Data object for ' +
              this.mapsvg_type +
              " '" +
              this.id +
              "'. Data object was set to empty object {}."
            this.data = {}
            return { _error: error }
          }
        } else {
  ...
  function Region(jQueryObject, globalOptions, regionID, mapsvg) {
    ...
    this.node[0].setAttribute(
      'class',
      (this.node.attr('class') || '') + ' mapsvg-region'
    )
    ...
    var regionOptions =
      globalOptions.regions && globalOptions.regions[this.id]
        ? globalOptions.regions[this.id]
        : null
  ...
  Region.prototype.getOptions = function (forTemplate) {
    ...
    var o = {
      ...
      title: this.title,
      disabled:
        this.disabled === this.getDisabledState(true)
          ? undefined
          : this.disabled,
      fill:
        this.globalOptions.regions[this.id] &&
        this.globalOptions.regions[this.id].fill,
  ...
  Region.prototype.setFill = function (fill) {
    ...
    if (this.globalOptions.gauge.on && this.gaugeValue) {
      ...
      this.default_attr['fill'] = this.style.fill
        ? this.style.fill
        : this.globalOptions.colors.baseDefault
    } else {
      this.default_attr['fill'] = 'none'
    }

    if (MapSVG.isNumber(this.globalOptions.colors.selected))
      this.selected_attr['fill'] = MapSVG.tinycolor(this.default_attr.fill)
        .lighten(parseFloat(this.globalOptions.colors.selected))
        .toRgbString()
    else this.selected_attr['fill'] = this.globalOptions.colors.selected

    if (MapSVG.isNumber(this.globalOptions.colors.hover))
      this.hover_attr['fill'] = MapSVG.tinycolor(this.default_attr.fill)
        .lighten(parseFloat(this.globalOptions.colors.hover))
        .toRgbString()
    else this.hover_attr['fill'] = this.globalOptions.colors.hover
    ...
    if (
      ...
      this.globalOptions.colors.stroke != undefined
    ) {
  ...
  Region.prototype.setStatus = function (status) {
    ...
    if (
      (statusOptions =
        ...
        this.globalOptions.regionStatuses[status])
    ) {
      this.status = status
  ...
  function Marker(options, scale, mapsvg) {
    ...
    var img = $('<img id="' + options.id + '" src="' + options.src + '"/>')
      .css({
    ...
    this.attached =
      options.attached == undefined
        ? true
        : MapSVG.parseBoolean(options.attached)
    this.isLink =
      options.isLink == undefined ? false : MapSVG.parseBoolean(options.isLink)
  ...
  Marker.prototype.update = function (data, mapScale) {
    for (var key in data) {
      // check if there's a setter for a property
      var setter = 'set' + MapSVG.ucfirst(key)
      if (setter in this) this[setter](data[key], mapScale)
    }
  }
  ...
  Marker.prototype.adjustPosition = function (mapScale) {
    ...
    this.node.css({
      transform:
        'translate(-50%,-100%) translate(' + pos[0] + 'px,' + pos[1] + 'px)',
    })
  }
  ...
  Marker.getDefaultCoords = function (
    markerX,
    markerY,
    markerWidth,
    markerHeight,
  ...
  Marker.prototype.drag = function (
    startCoords,
    scale,
    endCallback,
    clickCallback
    ...
    $('body').on('mouseup.drag.mapsvg', function (e) {
      ...
      if (_this.ox == _this.x && _this.oy == _this.y) clickCallback.call(_this)
    })
  }
  ...
  Marker.prototype.delete = function () {
    if (this.href) this.node.parent('a').empty().remove()
    this.mapsvg
      .getData()
      .$map.find('#' + this.id)
      .empty()
    ...
    this.methods = {
      ...
      setLayersControl: function (options) {
        ...
        if (_data.options.layersControl.on) {
          ...
          _data.$layersControl.removeClass(
            'mapsvg-top-left mapsvg-top-right mapsvg-bottom-left mapsvg-bottom-right'
          )
          _data.$layersControl.addClass(
            'mapsvg-' + _data.options.layersControl.position
          )
          // if(!_data.options.layersControl.expanded && !_data.$layersControl.hasClass('closed')){
          //     _data.$layersControl.addClass('closed')
          // }
          _data.$layersControl.css({
            'max-height': _data.options.layersControl.maxHeight,
          })

          _data.options.groups.forEach(function (g) {
            var item = $(
              '<div class="mapsvg-layers-item" data-group-id="' +
                g.id +
                '">' +
          ...
          _data.$layersControlListNano.nanoScroller({
          ...
          _data.$layersControl.toggleClass(
            'closed',
            !_data.options.layersControl.expanded
          )
        } else {
      ...
      setFloorsControl: function (options) {
        ...
        if (_data.options.floorsControl.on) {
          ...
          _data.$floorsControl.removeClass(
            'mapsvg-top-left mapsvg-top-right mapsvg-bottom-left mapsvg-bottom-right'
          )
          _data.$floorsControl.addClass(
            'mapsvg-' + _data.options.floorsControl.position
          )
          // if(!_data.options.floorsControl.expanded && !_data.$floorsControl.hasClass('closed')){
          //     _data.$floorsControl.addClass('closed')
          // }
          _data.$floorsControl.css({
            'max-height': _data.options.floorsControl.maxHeight,
          })

          _data.options.floors.forEach(function (f) {
            var item = $(
              '<div class="mapsvg-floors-item" data-floor-id="' +
                f.object_id +
                '">' +
                '<label>' +
                f.title +
          ...
          _data.$floorsControlListNano.nanoScroller({
            preventPageScrolling: true,
          })
          ...
          _data.$floorsControl.toggleClass(
            'closed',
            !_data.options.floorsControl.expanded
          )
        } else {
      ...
      setFloor: function (id) {
        _data.$floorsControl
          .find('.mapsvg-floors-item')
          .toggleClass('active', false)
        _data.$floorsControl
          .find('[data-floor-id="' + id + '"]')
      ...
      getPagination: function (callback) {
        ...
        _data.pager = $(
          '<nav class="mapsvg-pagination"><ul class="pager"><!--<li class="mapsvg-first"><a href="#">First</a></li>--><li class="mapsvg-prev"><a href="#">&larr; Prev. ' +
            _data.options.database.pagination.perpage +
            '</a></li><li class="mapsvg-next"><a href="#">Next ' +
            _data.options.database.pagination.perpage +
        ...
        if (_this.database.onFirstPage() && _this.database.onLastPage()) {
          ...
          _this.database.onLastPage() &&
            _data.pager.find('.mapsvg-next').addClass('disabled') &&
            _data.pager.find('.mapsvg-last').addClass('disabled')

          _this.database.onFirstPage() &&
            _data.pager.find('.mapsvg-prev').addClass('disabled') &&
            _data.pager.find('.mapsvg-first').addClass('disabled')
        }

        _data.pager
          .on('click', '.mapsvg-next:not(.disabled)', function (e) {
            e.preventDefault()
            if (_this.database.onLastPage()) return
            _this.database
              .getAll({ page: _this.database.page + 1 })
              .done(function () {
                callback && callback()
              })
          })
          .on('click', '.mapsvg-prev:not(.disabled)', function (e) {
            e.preventDefault()
            if (_this.database.onFirstPage()) return
            _this.database
              .getAll({ page: _this.database.page - 1 })
              .done(function () {
                callback && callback()
      ...
      functionFromString: function (string) {
        ...
        else return error //{error: {line: error.line, text: "MapSVG user function error: (line "+error.line+"): "+error.message}};
      },
      getOptions: function (forTemplate, forWeb, optionsDelta) {
        ...
        if (forTemplate) {
          ...
          if (_data.markers.length > 0) options.markers = []
      ...
      on: function (event, callback) {
        ...
        if (!_data.events[event]) _data.events[event] = []
      ...
      setEvents: function (functions) {
        ...
        for (var i in functions) {
          if (typeof functions[i] === 'string') {
            var func =
              functions[i] != '' ? this.functionFromString(functions[i]) : null

            if (
              func &&
              !func.error &&
              !(func instanceof TypeError || func instanceof SyntaxError)
            )
              _data.events[i] = func
            else _data.events[i] = null
      ...
      setRegionStatus: function (region, status) {
        var status =
          _this.regionsDatabase.getSchemaField('status').optionsDict[status]
        if (status.disabled) region.setDisabled(true)
        else region.setDisabled(false)

        if (status.color) region.setFill(status.color)
        else region.setFill()
      },
      update: function (options) {
        for (var key in options) {
          if (key == 'regions') {
            ...
            var setter = 'set' + MapSVG.ucfirst(key)
            if (_this.hasOwnProperty(setter)) this[setter](options[key])
            else {
      ...
      setRegionStatuses: function (_statuses) {
        ...
        for (var status in _data.options.regionStatuses) {
          colors[status] = _data.options.regionStatuses[status].color.length
            ? _data.options.regionStatuses[status].color
            : undefined
        }
      ...
      setColors: function (colors) {
        ...
        if (colors.status) _data.options.colors.status = colors.status
        ...
        $.each(_data.options.colors, function (key, color) {
          if (color === null || color == '') delete _data.options.colors[key]
        })
        if (colors.background)
          _data.$map.css({ background: _data.options.colors.background })
        if (colors.hover)
          _data.options.colors.hover =
            colors.hover == '' + parseInt(colors.hover)
              ? parseInt(colors.hover)
              : colors.hover
        if (colors.selected)
          _data.options.colors.selected =
            colors.selected == '' + parseInt(colors.selected)
              ? parseInt(colors.selected)
              : colors.selected
        _this.regionsRedrawColors()
      },
      setTooltips: function (options) {
        ...
        if (_data.tooltip.container) {
          _data.tooltip.container[0].className =
            _data.tooltip.container[0].className.replace(
              /(^|\s)mapsvg-tt-\S+/g,
              ''
            )
        } else {
        ...
        _data.tooltip.container.addClass(
          'mapsvg-tt-' + _data.options.tooltips.position
        )
        ...
        $('body')
          ...
          .on(event, function (e) {
            ...
            _data.tooltip.container[0].style.left =
              e.clientX +
              $(window).scrollLeft() -
              _data.$map.offset().left +
              'px'
            _data.tooltip.container[0].style.top =
              e.clientY + $(window).scrollTop() - _data.$map.offset().top + 'px'
            ...
            if (_data.tooltip.mirror.top || _data.tooltip.mirror.bottom) {
              ...
              if (_data.tooltip.mirror.top && m.y > _data.tooltip.mirror.top) {
                ...
                _data.tooltip.mirror.bottom &&
                m.y < _data.tooltip.mirror.bottom
              ) {
            ...
            if (_data.tooltip.mirror.right || _data.tooltip.mirror.left) {
              // may be cancel mirroring
              if (
                _data.tooltip.mirror.left &&
                m.x > _data.tooltip.mirror.left
              ) {
                _data.tooltip.mirror.left = false
                delete _data.tooltip.posShifted.leftright
              } else if (
                _data.tooltip.mirror.right &&
                m.x < _data.tooltip.mirror.right
              ) {
            ...
            if (
              _data.tooltip.posShifted.topbottom !=
                _data.tooltip.posOriginal.topbottom ||
              _data.tooltip.posShifted.leftright !=
                _data.tooltip.posOriginal.leftright
            ) {
              _data.tooltip.container[0].className =
                _data.tooltip.container[0].className.replace(
                  /(^|\s)mapsvg-tt-\S+/g,
      ...
      setPopovers: function (options) {
        if (
          ...
          options.mode.indexOf('function') == 0
        ) {
        ...
        _data.options.popovers.centerOn = MapSVG.parseBoolean(
          _data.options.popovers.centerOn
        )
        ...
        _data.$popover.css({
          width:
            _data.options.popovers.width +
            (_data.options.popovers.width == 'auto' ? '' : 'px'),
          'max-width': _data.options.popovers.maxWidth + '%',
          'max-height':
            (_data.options.popovers.maxHeight * _data.$map.outerHeight()) /
              100 +
            'px',
        })
      ...
      setInitialViewBox: function (v) {
        if (typeof v == 'string') v = v.trim().split(' ')
        _data._viewBox = [
          parseFloat(v[0]),
          parseFloat(v[1]),
          parseFloat(v[2]),
      ...
      setViewBox: function (v, skipAdjustments) {
        ...
        var isZooming =
          parseFloat(d[2]) != _data.viewBox[2] ||
          parseFloat(d[3]) != _data.viewBox[3]
        _data.viewBox = [
          parseFloat(d[0]),
          parseFloat(d[1]),
          parseFloat(d[2]),
        ...
        _data.superScale =
          (_data.whRatioFull * _data.svgDefault.viewBox[2]) / _data.viewBox[2]

        _data.scroll = _data.scroll || {}
        _data.scroll.tx =
          (_data.svgDefault.viewBox[0] - _data.viewBox[0]) * _data.scale
        _data.scroll.ty =
          (_data.svgDefault.viewBox[1] - _data.viewBox[1]) * _data.scale

        _data.$scrollpane.css({
          transform:
            'translate(' + _data.scroll.tx + 'px,' + _data.scroll.ty + 'px)',
        })
        _data.$svg.css({
      ...
      setViewBoxByGoogleMapBounds: function () {
        ...
        if (xyNE[0] < xySW[0]) {
          var mapPointsWidth =
            (_data.svgDefault.viewBox[2] / _data.mapLonDelta) * 360
      ...
      setSize: function (width, height, responsive) {
        ...
        if (!_data.options.width && !_data.options.height) {
          ...
          _data.options.width = parseInt(
            (_data.options.height * _data.svgDefault.width) /
              _data.svgDefault.height
          )
        } else if (_data.options.width && !_data.options.height) {
          _data.options.height = parseInt(
            (_data.options.width * _data.svgDefault.height) /
              _data.svgDefault.width
          )
        }
        ...
        if (_data.markers) _this.markersAdjustPosition()

        return [_data.options.width, _data.options.height]
      },
      setResponsive: function (on, force) {
        on =
          on != undefined ? MapSVG.parseBoolean(on) : _data.options.responsive
        ...
        if (MapSVG.browser.ie) {
          _data.whRatio2 =
            _data.svgDefault.viewBox[2] / _data.svgDefault.viewBox[3]
          _data.$svg.height(_data.$map.outerWidth() / _data.whRatio2)
        }
        if (!_data.resizeSensor) {
          _data.resizeSensor = new MapSVG.ResizeSensor(
            _data.$map[0],
            function () {
              if (MapSVG.browser.ie) {
                _data.whRatio2 =
              ...
              _data.$popover &&
                _data.$popover.css({
                  'max-height':
                    (_data.options.popovers.maxHeight *
                      _data.$map.outerHeight()) /
                      100 +
                    'px',
                })
              _this.updateSize()
      ...
      setScroll: function (options, skipEvents) {
        ...
        options.limit != undefined &&
          (options.limit = MapSVG.parseBoolean(options.limit))
          ...
          for (var i = 0; i <= 20; i++) {
            _data.zoomLevels[i + ''] = {
              ...
              viewBox: [
                0,
                0,
                _data._viewBox[2] / _scale,
                _data._viewBox[3] / _scale,
              ],
            }
          ...
          if (MapSVG.browser.firefox) {
            ...
            $(document).scroll(function () {
              if (_data.firefoxScroll.insideIframe)
                window.scrollTo(
                  _data.firefoxScroll.scrollX,
                  _data.firefoxScroll.scrollY
                )
            })
          }
      ...
      setZoomButtons: function () {
        ...
        _data.options.zoom.on && loc != 'hide'
          ? _data.zoomButtons.show()
          : _data.zoomButtons.hide()
      },
      setCursor: function (type) {
        type = type == 'pointer' ? 'pointer' : 'default'
        _data.options.cursor = type
        if (type == 'pointer') _data.$map.addClass('mapsvg-cursor-pointer')
        else _data.$map.removeClass('mapsvg-cursor-pointer')
      },
      ...
      setMultiSelect: function (on, deselect) {
        ...
        if (deselect !== false) _this.deselectAllRegions()
      },
      setGauge: function (options) {
        ...
        if (!_data.$gauge) {
          _data.$gauge = {}
          _data.$gauge.gradient = $('<td>&nbsp;</td>').addClass(
            'mapsvg-gauge-gradient'
          )
          ...
          _data.$gauge.labelLow = $(
            '<td>' + _data.options.gauge.labels.low + '</td>'
          )
          _data.$gauge.labelHigh = $(
            '<td>' + _data.options.gauge.labels.high + '</td>'
          )
        ...
        if (!_data.options.gauge.on && _data.$gauge.container.is(':visible')) {
          ...
          _data.options.gauge.on &&
          !_data.$gauge.container.is(':visible')
        ) {
        ...
        if (options.colors) {
          _data.options.gauge.colors.lowRGB = MapSVG.tinycolor(
            _data.options.gauge.colors.low
          ).toRgb()
          _data.options.gauge.colors.highRGB = MapSVG.tinycolor(
            _data.options.gauge.colors.high
          ).toRgb()
          _data.options.gauge.colors.diffRGB = {
            r:
              _data.options.gauge.colors.highRGB.r -
              _data.options.gauge.colors.lowRGB.r,
            g:
              _data.options.gauge.colors.highRGB.g -
              _data.options.gauge.colors.lowRGB.g,
            b:
              _data.options.gauge.colors.highRGB.b -
              _data.options.gauge.colors.lowRGB.b,
            a:
              _data.options.gauge.colors.highRGB.a -
              _data.options.gauge.colors.lowRGB.a,
          }
          needsRedraw = true
      ...
      updateGaugeMinMax: function () {
        ...
        if (values.length > 0) {
          _data.options.gauge.min =
            values.length == 1 ? 0 : Math.min.apply(null, values)
          _data.options.gauge.max = Math.max.apply(null, values)
          _data.options.gauge.maxAdjusted =
            _data.options.gauge.max - _data.options.gauge.min
        }
      },
      setGaugeGradientCSS: function () {
        _data.$gauge.gradient.css({
          ...
          background:
            ...
            ' 100%)',
          background:
            '-webkit-gradient(linear, left top, right top, color-stop(1%,' +
            _data.options.gauge.colors.low +
            '), color-stop(100%,' +
            _data.options.gauge.colors.high +
            '))',
      ...
      setMenu: function (options) {
        ...
        if (_data.options.menu.on) {
          ...
          if (menuContainer.length) {
            ...
            _data.$menu
              .on('click.menu.mapsvg', 'a', function (e) {
                e.preventDefault()
          ...
          if (!MapSVG.googleMapsApiLoaded) {
            ...
            if (!_data.googleMaps.map) {
              ...
              USGSOverlay.prototype.draw = function () {
                ...
                var sw2 = overlayProjection.fromLatLngToContainerPixel(
                  this.bounds_.getSouthWest()
                )
                var ne2 = overlayProjection.fromLatLngToContainerPixel(
                  this.bounds_.getNorthEast()
                )
              ...
              var southWest = new google.maps.LatLng(
                _data.geoViewBox.bottomLat,
                _data.geoViewBox.leftLon
              )
              var northEast = new google.maps.LatLng(
                _data.geoViewBox.topLat,
                _data.geoViewBox.rightLon
              )
              var bounds = new google.maps.LatLngBounds(southWest, northEast)

              _data.googleMaps.overlay = new USGSOverlay(
                bounds,
                _data.googleMaps.map
              )

              if (
                !_data.options.googleMaps.center ||
                !_data.options.googleMaps.zoom
              ) {
                var southWest = new google.maps.LatLng(
                  _data.geoViewBox.bottomLat,
                  _data.geoViewBox.leftLon
                  ...
                  setTimeout(function () {
                    ...
                    setTimeout(function () {
                      ...
                      if (
                        ...
                        _data.options.googleMaps.center = _data.googleMaps.map
                          .getCenter()
                          .toJSON()
                        _data.options.googleMaps.zoom =
                          _data.googleMaps.map.getZoom()
                      }
                    }, 300)
                  }, 1)
                }
              ...
              setTimeout(function () {
                _data.googleMaps.map.addListener('bounds_changed', function () {
                  // _data.googleMaps.overlay.draw();
                  setTimeout(function () {
                    if (!_data.isScrolling) _data.googleMaps.overlay.draw()
                    // _this.setViewBoxByGoogleMapBounds();
                  }, 100)
                })
      ...
      loadGoogleMapsAPI: function (callback, fail) {
        ...
        _data.googleMapsScript.src =
          ...
          _data.options.googleMaps.apiKey +
          '&v=3.31' //+'&callback=initMap';
      ...
      loadDetailsView: function (obj) {
        ...
        if (_this.detailsController) _this.detailsController.destroy()
        ...
        _this.detailsController = new MapSVG.DetailsController({
          ...
          template:
            obj instanceof Region
              ? _data.templates.detailsViewRegion
              : _data.templates.detailsView,
          mapsvg: _this,
        ...
        if (_data.options.menuMarkers.on) {
          ...
          if (menuContainer.length) {
            ...
            _data.markers.forEach(function (marker, i) {
              _data.$menuMarkers.append(
                _data.options.menuMarkers.template(marker)
              )
            })

            _data.$menuMarkers
              .on('click.menuMarkers.mapsvg', 'a', function (e) {
                e.preventDefault()
              ...
              .on('mouseover.menuMarkers.mapsvg', 'a', function (e) {
                ...
                var markerID = $(this).attr('href').replace('#', '')
                var marker = _this.getMarker(markerID)
                _data.options.mouseOver &&
                  _data.options.mouseOver.call(marker, e, _this)
              })
              .on('mouseout.menuMarkers.mapsvg', 'a', function (e) {
                e.preventDefault()
                var markerID = $(this).attr('href').replace('#', '')
                var marker = _this.getMarker(markerID)
                _data.options.mouseOut &&
                  _data.options.mouseOut.call(marker, e, _this)
              })
          }
        }
      },
      ...
      viewBoxGetBySize: function (width, height) {
        ...
        if (new_ratio != old_ratio) {
          ...
          if (new_ratio > old_ratio) {
            ...
            vb[0] =
              _data.svgDefault.viewBox[0] -
              (vb[2] - _data.svgDefault.viewBox[2]) / 2
          } else {
            vb[3] = _data.svgDefault.viewBox[2] / new_ratio
            vb[1] =
              _data.svgDefault.viewBox[1] -
              (vb[3] - _data.svgDefault.viewBox[3]) / 2
          }
        }
        ...
        _data.$svg
          ...
          .each(function (index) {
            if ($(this).data('stroke-width')) {
              $(this).css(
                'stroke-width',
                $(this).data('stroke-width') / _data.scale
              )
            }
          })
      },
      ...
      zoomOut: function (center) {
        if (_data.googleMaps.map) {
          if (
            !_data.isZooming &&
            _data.googleMaps.map.getZoom() - 1 >=
              _data.options.googleMaps.minZoom
          ) {
            _data.isZooming = true
            var zoom = _data.googleMaps.map.getZoom() - 1
            zoom = zoom < 1 ? 1 : zoom
      ...
      zoomTo: function (region, zoomToLevel) {
        ...
        $.each(_data.zoomLevels, function (key, level) {
          if (viewBoxPrev.length) {
            if (
              bbox[2] > bbox[3] &&
              viewBoxPrev[2] > bbox[2] &&
              bbox[2] > level.viewBox[2]
            ) {
              ...
              bbox[2] < bbox[3] &&
              viewBoxPrev[3] > bbox[3] &&
              bbox[3] > level.viewBox[3]
            ) {
              _data.zoomLevel = zoomToLevel ? zoomToLevel : parseInt(key) - 1
              var vb = _data.zoomLevels[_data.zoomLevel].viewBox
              _this.setViewBox([
      ...
      zoom: function (delta, center, exact) {
        ...
        var isInZoomRange =
          _data.zoomLevel >= _data.options.zoom.limit[0] &&
          _data.zoomLevel <= _data.options.zoom.limit[1]

        if (!exact) {
          // check for zoom limit
          var d = delta > 0 ? 1 : -1

          if (!_data.zoomLevels[_data.zoomLevel + d]) return
          ...
          if (
            isInZoomRange &&
            (_data._zoomLevel > _data.options.zoom.limit[1] ||
              _data._zoomLevel < _data.options.zoom.limit[0])
          )
            return false
        ...
        if (center) {
          ...
          shift = [
            (center[0] - _data.viewBox[0]) * koef,
            (center[1] - _data.viewBox[1]) * koef,
          ]
        ...
        if (_data.options.scroll.limit) {
          if (newViewBox[0] < _data.svgDefault.viewBox[0])
            ...
            newViewBox[0] + newViewBox[2] >
            _data.svgDefault.viewBox[0] + _data.svgDefault.viewBox[2]
          )
            newViewBox[0] =
              _data.svgDefault.viewBox[0] +
              _data.svgDefault.viewBox[2] -
              newViewBox[2]

          if (newViewBox[1] < _data.svgDefault.viewBox[1])
            newViewBox[1] = _data.svgDefault.viewBox[1]
          else if (
            newViewBox[1] + newViewBox[3] >
            _data.svgDefault.viewBox[1] + _data.svgDefault.viewBox[3]
          )
            newViewBox[1] =
              _data.svgDefault.viewBox[1] +
              _data.svgDefault.viewBox[3] -
              newViewBox[3]
        }
      ...
      markerDelete: function (marker) {
        ...
        if (_data.markers.length == 0) _data.options.markerLastID = 0
      },
      // MARK : ADD
      markerAdd: function (opts, create) {
        ...
        if (options.width && options.height) {
          if (options.geoCoords) {
            ...
            options.xy ||
            (MapSVG.isNumber(options.x) && MapSVG.isNumber(options.y))
          ) {
      ...
      markerId: function () {
        ...
        if (_data.markersDict[id] != undefined) return _this.markerId()
        else return id
      },
      ...
      scrollStart: function (e, mapsvg) {
        if (
          $(e.target).hasClass('mapsvg-btn-zoom') ||
          $(e.target).closest('.mapsvg-gauge').length
        )
          return false

        if (
          _data.editMarkers.on &&
          $(e.target).hasClass('class') == 'mapsvg-marker'
        )
          return false
      ...
      scrollMove: function (e) {
        ...
        if (_data.options.scroll.limit) {
          if (vx < _data.svgDefault.viewBox[0]) vx = _data.svgDefault.viewBox[0]
          else if (
            _data.viewBox[2] + vx >
            _data.svgDefault.viewBox[0] + _data.svgDefault.viewBox[2]
          )
          ...
          if (vy < _data.svgDefault.viewBox[1]) vy = _data.svgDefault.viewBox[1]
          else if (
            _data.viewBox[3] + vy >
            _data.svgDefault.viewBox[1] + _data.svgDefault.viewBox[3]
          )
      ...
      scrollEnd: function (e, mapsvg, noClick) {
        ...
        if (
          noClick !== true &&
          Math.abs(_data.scroll.dx) < 5 &&
          Math.abs(_data.scroll.dy) < 5
        ) {
      ...
      touchStart: function (_e, mapsvg) {
        if (
          $(_e.target).hasClass('mapsvg-popover') ||
          $(_e.target).closest('.mapsvg-popover').length
        ) {
          return true
        }
        _e.preventDefault()
          ...
          if (e.scale != 1 && _data.canZoom) {
            ...
            var cy =
              e.touches[0].pageY >= e.touches[1].pageY
                ? e.touches[0].pageY -
                  (e.touches[0].pageY - e.touches[1].pageY) -
                  _data.$svg.offset().top
                : e.touches[1].pageY -
                  (e.touches[1].pageY - e.touches[0].pageY) -
                  _data.$svg.offset().top
            var center = _this.convertPixelToSVG([cx, cy])

            if (d > 0) _this.zoomIn(center)
            else _this.zoomOut(center)
          }
      ...
      deselectRegion: function (region) {
        if (!region) region = _this.getRegion(_data.selected_id[0])
        if (region) {
          region.deselect()
      ...
      unhighlightRegions: function () {
        _data.highlightedRegions.forEach(function (region) {
          if (!region.selected && !region.disabled) region.unhighlight()
        })
      ...
      convertSVGToPixel: function (xy) {
        ...
        return [
          (xy[0] - _data.svgDefault.viewBox[0]) * scale,
          (xy[1] - _data.svgDefault.viewBox[1]) * scale,
        ]
      },
      convertPixelToSVG: function (xy) {
        var scale = _this.getScale()
        return [
          xy[0] / scale + _data.svgDefault.viewBox[0],
          xy[1] / scale + _data.svgDefault.viewBox[1],
        ]
      },
      convertGeoToSVG: function (coords) {
        ...
        var x =
          (lon - _data.geoViewBox.leftLon) *
          (_data.svgDefault.viewBox[2] / _data.mapLonDelta)
        ...
        var mapOffsetY =
          (worldMapWidth / 2) *
          Math.log(
            (1 + Math.sin(_data.mapLatBottomDegree)) /
              (1 - Math.sin(_data.mapLatBottomDegree))
          )
        var y =
          _data.svgDefault.viewBox[3] -
      ...
      convertSVGToGeo: function (tx, ty) {
        ...
        var lon =
          _data.geoViewBox.leftLon +
          (tx / _data.svgDefault.viewBox[2]) * _data.mapLonDelta
      ...
      pickGaugeColor: function (gaugeValue) {
        var w =
          (gaugeValue - _data.options.gauge.min) /
          _data.options.gauge.maxAdjusted
        var rgb = [
          Math.round(
            _data.options.gauge.colors.diffRGB.r * w +
              _data.options.gauge.colors.lowRGB.r
          ),
          Math.round(
            _data.options.gauge.colors.diffRGB.g * w +
              _data.options.gauge.colors.lowRGB.g
          ),
          Math.round(
            _data.options.gauge.colors.diffRGB.b * w +
              _data.options.gauge.colors.lowRGB.b
          ),
          Math.round(
            _data.options.gauge.colors.diffRGB.a * w +
              _data.options.gauge.colors.lowRGB.a
          ),
        ]
        return rgb
      ...
      isRegionDisabled: function (id, svgfill) {
        if (
          _data.options.regions[id] &&
          (_data.options.regions[id].disabled || svgfill == 'none')
        ) {
          return true
        } else if (
          (_data.options.regions[id] == undefined ||
            MapSVG.parseBoolean(_data.options.regions[id].disabled)) &&
          (_data.options.disableAll ||
            svgfill == 'none' ||
            id == 'labels' ||
            id == 'Labels')
        ) {
      ...
      regionClickHandler: function (e, region, skipPopover) {
        ...
        if (region.mapsvg_type == 'region') _this.selectRegion(region.id)
        ...
        popover && _this.showPopover(region)

        if (_data.options.onClick) _data.options.onClick.call(region, e, _this)
      ...
      search: function (str) {
        ...
        _data.regions.forEach(function (r) {
          if (
            r.id.toLowerCase().indexOf(str) === 0 ||
            r.id.toLowerCase().indexOf('-' + str) !== -1 ||
            (r.title && r.title.toLowerCase().indexOf(str) === 0)
          )
      ...
      searchMarkers: function (str) {
        ...
        str = str.toLowerCase()
        _data.markers.forEach(function (m) {
          if (m.id.toLowerCase().indexOf(str) === 0) results.push(m.id)
        })
        return results
      },
      searchData: function (field, str) {
        ...
        _data.options.data.forEach(function (params) {
          for (var i in params) {
            if (
              ('' + params[i]).toLowerCase().indexOf(str) === 0 &&
              results.indexOf(params.id) == -1
            )
      ...
      hideMarkersExceptOne: function (id) {
        _data.markers.forEach(function (m) {
          if (m.id != id) m.node.addClass('mapsvg-marker-hidden')
        })
      },
      ...
      markerAddClickHandler: function (e) {
        ...
        if (window.defaultMarkerImage) data.src = window.defaultMarkerImage
      ...
      runUserFunction: function (func) {
        try {
          ...
          console.log(
            'MapSVG user-defined function error: (line ' +
              error.line +
              '): ' +
              error.message
          )
      ...
      download: function () {
        if (!_data.downloadForm) {
          _data.downloadForm = $(
            '<form id="mdownload" action="/wp-content/plugins/mapsvg-dev/download.php" method="POST"><input type="hidden" name="svg_file" value="0" /><input type="hidden" name="svg_title"></form>'
          )
          _data.downloadForm.appendTo('body')
        }
        _data.downloadForm
          .find('input[name="svg_file"]')
          .val(_data.$svg.prop('outerHTML'))
        _data.downloadForm
          .find('input[name="svg_title"]')
          .val(_data.options.title)
        setTimeout(function () {
      ...
      showTooltip: function (region) {
        ...
        if (_data.options.tooltips.priority == 'global') {
          tip =
            (typeof _data.options.tooltips.mode == 'function' &&
              _data.options.tooltips.mode.call(
                region,
                _data.tooltip.container,
                region,
                _this
              )) ||
            (_data.options.tooltips.mode != 'off' &&
              region[_data.options.tooltips.mode]) ||
            region.tooltip
        } else {
          tip =
            region.tooltip ||
      ...
      getPopoverBody: function (region) {
        ...
        if (_data.options.popovers.priority == 'global') {
          popover =
            typeof _data.options.popovers.mode == 'function'
              ? _data.options.popovers.mode.call(
                  region,
                  _data.mapPopover,
                  region,
                  _this
      ...
      showPopover: function (object) {
        ...
        var mapObject =
          object instanceof Region
            ? object
            : object.marker && object.marker.id
            ? _this.getMarker(object.marker.id)
            : null
        if (!mapObject) return

        var _this = this

        var template = _this.getPopoverBody(object)
          ...
          events: {
            shown: function () {
              if (_data.options.popovers.centerOn) {
                ...
                if (
                  _data.options.popovers.centerOn &&
                  !(MapSVG.isPhone && _data.options.popovers.mobileFullscreen)
                ) {
            ...
            closed: function () {
              _data.options.popovers.resetViewboxOnClose &&
      ...
      mouseOverHandler: function (e) {
        ...
        return (
          _data.options.mouseOver &&
          _data.options.mouseOver.call(this, e, _this)
        )
      },
      mouseOutHandler: function (e) {
        ...
        //if(!_data.editRegions.on && !_data.editMarkers.on)
        return (
          _data.options.mouseOut && _data.options.mouseOut.call(this, e, _this)
        )
      },
      ...
      setEventHandlers: function () {
          ...
          _data.$map.on(
            ...
            function (e) {
              ...
              marker.drag(
                ...
                function () {
                  if (_data.mapIsGeo) {
                    this.geoCoords = _this.convertSVGToGeo(
                      this.x + this.width / 2,
                      this.y + (this.height - 1)
                    )
                  }
                  _data.markerEditHandler &&
        ...
        if (_data.options.scroll.spacebar) {
            ...
            _data.$map.on(
              ...
              function (e) {
                ...
                if (
                  ...
                  _this.regionClickHandler.call(
                    _this,
                    e,
                    _this.getRegion($(this).attr('id'))
                  )
                }
                // _this.regionClickHandler.call(_this, e, _this.getRegion($(this).attr('id')));
              }
          ...
          _data.$map
            ...
            .on('touchmove.common.mapsvg', function (e) {
              e.preventDefault()
              _this.touchMove(e, _this)
            })
            .on('touchend.common.mapsvg', function (e) {
              e.preventDefault()
      ...
      addLayer: function (name) {
        _data.layers[name] = $(
          '<div class="mapsvg-layer mapsvg-layer-' + name + '"></div>'
        )
      ...
      init: function (opts, elem) {
        ...
        _data.map = elem
        _data.$map = $(elem)
        _data.$scrollpane = $('<div class="mapsvg-scrollpane"></div>').appendTo(
          _data.$map
        )
        _data.$layers = $('<div class="mapsvg-layers-wrap"></div>').appendTo(
          _data.$scrollpane
        )
        ...
        ;(_data.geoCoordinates = false),
          ...
          _data.$map
            ...
            .css('background', _data.options.colors.background)
        ...
        if (!_data.editMode) {
          ...
          if (_data.options.detailsView.location != 'custom') {
            ...
            if (
              !_data.options.menu.customContainer &&
              _data.options.detailsView.location == 'near'
            ) {
              _data.$details.css({ left: _data.options.menu.width })
              _data.$details.addClass('near')
            } else if (_data.options.detailsView.location == 'top') {
        ...
        var loading = $(
          '<div>' + _data.options.loadingText + '</div>'
        ...
        if (
          _data.options.extension &&
          $().mapSvg.extensions &&
          $().mapSvg.extensions[_data.options.extension]
        ) {
        ...
        $.ajax({
          url: _data.options.source + '?v=' + _data.options.svgFileVersion,
          ...
          if (_data.svgDefault.width && _data.svgDefault.height) {
            _data.svgDefault.width = parseFloat(
              _data.svgDefault.width.replace(/px/g, '')
            )
          ...
          var geo =
            svgTag.attr('mapsvg:geoViewBox') || svgTag.attr('mapsvg:geoviewbox')
          if (geo) {
            geo = geo.split(' ')
            if (geo.length == 4) {
              _data.mapIsGeo = true
              ...
              _data.mapLonDelta =
                _data.geoViewBox.rightLon - _data.geoViewBox.leftLon
              _data.mapLatBottomDegree =
                (_data.geoViewBox.bottomLat * 3.14159) / 180
            }
          }
          ...
          _data._viewBox =
            (_data.options.viewBox.length == 4 && _data.options.viewBox) ||
            _data.svgDefault.viewBox
          ...
          if (_data.options.cursor) _this.setCursor(_data.options.cursor)
          ...
          if (
            (match = RegExp('[?&]mapsvg_select=([^&]*)').exec(
...
function plans() {
  ...
  var $map = $('#mapsvg')
  var image1 = $map.data('image1')
  var image2 = $map.data('image2')

  if (bheight > 900) {
    $map.mapSvg({
      width: 100,
      height: 100,
      viewBox: [0, 0, 1350, 690],
      cursor: 'pointer',
      zoom: {
        on: true,
        limit: [0, 1.6],
  ...
  if (document.querySelector('.plan-item')){
    ...
    document.getElementById('mapsvg')
        .addEventListener('wheel', (e) => {
              ...
              if (e.deltaY < 0) {
                document.querySelector('.mapsvg-btn-zoom.in').click()
              } else {
                document.querySelector('.mapsvg-btn-zoom.out').click()
              }
            }, { passive: false }
        );
  }
